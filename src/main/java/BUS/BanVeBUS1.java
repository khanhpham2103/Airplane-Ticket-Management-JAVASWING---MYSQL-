...
//        System.out.println(BusNum.substring(0, BusNum.indexOf('/') )+ " "+ EcoNum.substring(0, EcoNum.indexOf('/') - 1));
        ...
        System.out.println(" Ma Nhan Vien " + String.valueOf(CommonVaribleDTO.nhanvien_st));
        HoaDonDTO dtotmp = new HoaDonDTO(MaHoaDon, CommonVaribleDTO.nhanvien_st, cus, Total, date);
...
// ins hoa don chi tiet vs update chuyen bay 
            addTicket(cus, tmp.getFlightID(), MaHoaDon, tick);
        }
    ...
    public ArrayList<String> GetMaxPerType() {
        BanVeDAO dao = new BanVeDAO();
        ArrayList<String> ds = dao.GetMaxPerType();
        ArrayList<String> res = new ArrayList<>();
        for (String get : ds) {
            String lastID = get.substring(3);
    ...
    private String IncreaseID(String Name, int id) {
        if (id < 10) {
            id++;
            return Name + "00" + id;
        }
    ...
    public void addTicket(String MaKH, String MaCB, String MaHD, BanVeDTO bv1) {
        ticketBUS bustk = new ticketBUS();
        typeBUS bustype = new typeBUS();
        String BusID = "", EcoID = "";
        int EcoNum = Integer.parseInt(bv1.getEcoNum());
        int BusNum = Integer.parseInt(bv1.getBusNum());
        bustk.getList2();
        bustype.getList2();
//        ArrayList<ticketDTO> dto = bustk.getList();
//        ticketDTO last = dto.get(dto.size() - 1);
//        String lastID = last.getTicketID().substring(4);
//        int NewID = Integer.parseInt(lastID);
//        int NewID = 180;
        ArrayList<String> dto = GetMaxPerType();
        int busid = Integer.parseInt(dto.get(0));
        int ecoid = Integer.parseInt(dto.get(1));

        for (int i = 0; i < EcoNum; i++) {
            String idTic = IncreaseID("ECO", ecoid);
            ticketDTO tmp = new ticketDTO(idTic, MaKH, MaCB, "ECO", MaHD);
            bustk.add(tmp);
            ecoid++;
        }
        for (int i = 0; i < BusNum; i++) {
            String idTic = IncreaseID("BUS", busid);
            ticketDTO tmp = new ticketDTO(idTic, MaKH, MaCB, "BUS", MaHD);
            bustk.add(tmp);
            busid++;
        }
        System.out.println(busid + " "+ ecoid +" " + " " + MaKH + " " + MaCB + " " + MaHD + " " + bv1.getBusNum() + " " + bv1.getEcoNum());
    }
    ...
    public void addInvoiceDetail(BanVeDTO tick, ChuyenBayDTO cb, HoaDonDTO hd, int BookedEco, int BookedBus, int ToTalBus, int ToTalEco) {
        ...
        System.out.println("So luong Bus Eco " + BusNum + " " + EcoNum);
        if (BusNum > 0) {
