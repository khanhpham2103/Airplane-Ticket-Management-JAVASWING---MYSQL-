...
public class flightView extends javax.swing.JFrame {
    private DefaultTableModel tblModelFlight;
    ...
    protected void initTable() {
        ...
        tblModelFlight = new DefaultTableModel();
        tblModelFlight.setColumnIdentifiers(new String[]{"Flight Id", "Route Id", "Airplane Id", "Flight Schedule Id", "Departure Time", "Arrival Time", "Total Seats", "Booked ECO Seats", "Booked BUS Seats"});
        tblFlight.setModel(tblModelFlight);
...
//        setCellsAlignment(tblTicket, SwingConstants.CENTER);
    ...
    private void loadTableType() {
        ...
        ArrayList<ChuyenBayDTO> typeList = CBBUS.getList();

        tblModelFlight.setRowCount(0);
        for (ChuyenBayDTO CB : typeList) {
            String[] row = new String[]{
                CB.getFlightID(), CB.getRouteID(), CB.getAirplaneID(), CB.getFlightScheduleID(), CB.getDepartureTime(), CB.getArrivalTime(), String.valueOf(CB.getTotalSeats()), CB.getBookedECOSaets(), CB.getBookedBUSSeats()
            };
            tblModelFlight.addRow(row);
        }
        tblModelFlight.fireTableDataChanged();
    }
